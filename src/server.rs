...
pub async fn start() {
    ...
    let db_config = crate::database::connection::DatabaseConfig::from_env()
        .expect("Failed to load DB config from env");
    let db = Arc::new(
        crate::database::connection::DatabaseConnection::new(db_config)
            .await
    ...
    let allowed_origins = AllowOrigin::list(vec![
        "https://fr-icm-ui.vercel.app".parse::<HeaderValue>().unwrap(),
